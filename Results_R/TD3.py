...
class Actor(nn.Module):
    def __init__(self, in_dim: int, out_dim: int, init_w: float = 5e-3):
        ...
        self.bn_input = nn.BatchNorm1d(6)
...
class TD3:
    ...
    def __init__(
            ...
            exploration_noise: float = 0.5,  # 0.5
            target_policy_noise: float = 0.2,
            target_policy_noise_clip: float = 0.1,
        ...
        """Initialize."""
        self.memory = PrioritizedReplayBuffer(obs_dim, memory_size, batch_size, alpha=0.7)
        ...
        self.scheduler = torch.optim.lr_scheduler.StepLR(self.critic_optimizer, step_size=100, gamma=0.9)
        ...
        self.is_test = False
        self.total_step = 0
        self.update_multiplier_every_n_batches = 10
        self.multiplier_history = {'roi_diff': [], 'rbr': []}
        log_mult = []
        log_mult1 = []
        ...
        self.multiplier_optimizer = torch.optim.Adam([x for x in log_mult if x is not None], lr=0.01)  # 0.01
    ...
    def update_model(self) -> Tuple:
        ...
        states = torch.as_tensor(sample["obs"], dtype=torch.float32).to(self.device)
        next_states = torch.as_tensor(sample["next_obs"], dtype=torch.float32).to(self.device)
        actions = torch.as_tensor(sample["acts"].reshape(-1, 1), dtype=torch.float32).to(self.device)

        rewards = torch.as_tensor(sample["rews"].reshape(-1, 1), dtype=torch.float32).to(self.device)
        dones = torch.as_tensor(sample["done"].reshape(-1, 1), dtype=torch.float32).to(self.device)
        masks = 1 - dones

        self.beta = min(self.beta_max, self.beta + self.beta_step)

        # get actions with noise
        noise = torch.as_tensor(self.target_policy_noise.sample(), dtype=torch.float32).to(self.device)
        ...
        # next_state_numpy = next_states.cpu().numpy()
        # next_actions = self.select_action(next_state_numpy)
        # next_actions = next_actions + clipped_noise
        # next_actions = torch.full((next_states.shape[0], 1), next_actions, dtype=torch.float32).to(self.device)
