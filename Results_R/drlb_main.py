...
def get_budget(train_data):
    _ = []
    for index, day in enumerate(train_data[4].unique()):
        current_day_budget = np.sum(train_data[train_data[4].isin([day])][3])
            ...
            for i in range(len(slot_data)):
                ...
                if current_day_imp_num[slot] - temp_slot_real_imp >= 0:
                    if current_day_budget[slot] - temp_slot_cost >= 0:
                        ...
                        if slot_data[i, 3] <= bid_price_array[i]:
                            # 如果出价大于了市场价
                            if slot_data[i, 2]  / slot_data[i, 3]  >= roi:
                            # 如果当前ROI约束满足

                                temp_slot_cost += slot_data[i, 3]
                                temp_slot_win_imp += 1
                                temp_slot_win_clk += slot_data[i, 0]
                                temp_slot_reward += (slot_data[i, 2] - slot_data[i, 3]) # 奖励函数设置为效用即v-D
...
def state_(init_budget,
    ...
    # condition_2 = np.any((slot_data[:, 2] - max(2, slot_data[:, 3])) / max(2, slot_data[:, 3]) >= roi)
    # final_condition = np.logical_and(condition_1, condition_2)
    #win_data = slot_data[slot_data[:, 3] <= slot_bid_price_array]  # 筛选赢标的数据
    mask = (slot_data[:, 3] <= slot_bid_price_array) & (
            slot_data[:, 2]  / ( 1e-3+ slot_data[:, 3]) >= roi)
    win_data = slot_data[np.where(mask)]

    # 总体统计
    slot_cost = np.sum(win_data[:, 3])  # 当前时段花费
    ...
    slot_reward = np.sum(win_data[:, 2] - win_data[:, 3])  # 记录效用
    slot_value=np.sum(win_data[:, 2])
    ...
    # slot_market_price=np.sum(slot_data[:, 3])

    # slot_ROI = (slot_cost - slot_market_price) /  slot_market_price
...
def choose_init_lambda(config, budget_para):
    base_bid_path = os.path.join('../lin_2/result1/ipinyou/{}/normal/test'.format(config['campaign_id']),
                                 'test_bid_log.csv')
...
def run_train(budget_para):
    ...
    # 0:clk, 1:pctr, 2:value,3:market_price, 4:day, 5:time_fraction
    train_data.columns = [0, 1, 2, 3, 4, 5]

    # 转换数据格式
    train_data.iloc[:, [0, 3, 4, 5]] = train_data.iloc[:, [0, 3, 4, 5]].astype(int)
    train_data.iloc[:, [1, 2]] = train_data.iloc[:, [1, 2]].astype(float)
    ...
    for epoch in range(config['train_epochs']):
        ...
        for day_index, day in enumerate(train_data[4].unique()):
            current_day_data = train_data[train_data[4].isin([day])]
            ...
            for t in range(config['time_fraction']):
                ...
                # current_slot_data[2] value 估值
                #
                # current_slot_data[3] 市场价格
                #
                # current_slot_data[4] 天
                # current_slot_data[5]时段
                current_slot_data = current_day_data[current_day_data.iloc[:, 5].isin([slot])]  # 当前时段的数据
                ...
                if t == 0:
                    ...
                    # 第1个时段的数据（第二个时段）
                    next_slot_data = current_day_data[current_day_data.iloc[:, 5].isin([slot + 1])]  # 下一时段的数据
                    ...
                    next_slot_data = current_day_data[current_day_data.iloc[:, 5].isin([slot + 1])]  # t时段的数据
                    next_slot_data_pctr = next_slot_data.iloc[:, 1].values  # ctrs
                    next_slot_data_value = next_slot_data.iloc[:, 2].values  # 估值
                ...
                current_day_win_utility.append(slot_reward)  # 是估值和市场价值之间的差
            ...
            episode_win_utility.append(np.sum(current_day_win_utility))

            episode_lambda.append(current_day_lambda)
            episode_action.append(current_day_action)

        print('训练：真实曝光数{}, 赢标数{}, 共获得{}个点击, 真实点击数{}, 获得pCTR{}, 获得效用{}, 预算{}, 花费{}, CPM{}, 其次 CPC{}'
              .format(np.sum(episode_real_imps),
                      ...
                      np.sum(episode_win_utility),
                      np.sum(total_budget),
                      np.sum(episode_cost),
                      np.sum(episode_cost) / np.sum(episode_win_imps) if np.sum(episode_win_imps) > 0 else 0,
                      np.sum(episode_cost) / np.sum(episode_win_clks) if np.sum(episode_win_clks) > 0 else 0,
        ...
        episode_result_data = [np.sum(episode_real_imps), np.sum(episode_win_imps), np.sum(episode_win_clks),
                               np.sum(episode_real_clks), np.sum(episode_win_pctr), np.sum(episode_win_utility),
                               np.sum(total_budget),
                               np.sum(episode_cost),
                               np.sum(episode_cost) / np.sum(episode_win_imps) if np.sum(episode_win_imps) > 0 else 0,
                               np.sum(episode_cost) / np.sum(episode_win_clks) if np.sum(episode_win_clks) > 0 else 0,
    ...
    columns = ['real_imps', 'win_imps', 'clks', 'real_clks', 'pctr', 'utility', 'budget', 'spent', 'CPM', 'CPC']
...
def run_test(budget_para):
    ...
    test_data.columns = [0, 1, 2, 3, 4, 5]

    # 转换数据格式
    test_data.iloc[:, [0, 3, 4, 5]] = test_data.iloc[:, [0, 3, 4, 5]].astype(int)
    test_data.iloc[:, [1, 2]] = test_data.iloc[:, [1, 2]].astype(float)
    ...
    for day_index, day in enumerate(test_data[4].unique()):
        current_day_data = test_data[test_data[4].isin([day])]
        ...
        for t in range(config['time_fraction']):
            ...
            current_day_real_clks.append(slot_real_clk)
            current_day_win_pctr.append(slot_reward)
            current_day_win_utility.append(slot_reward)
    ...
    print('测试：真实曝光数{}, 赢标数{}, 共获得{}个点击, 真实点击数{}, 获得pCTR{},获得效用, 预算{}, 花费{}, CPM{}, CPC{},'
          .format(np.sum(episode_real_imps),
                  np.sum(episode_win_imps),
                  np.sum(episode_win_clks),
                  np.sum(episode_real_clks),
...
if __name__ == '__main__':
    ...
    parser.add_argument('--budget_para', default=[2,4,8,16,32])
    ...
    for i in range(len(budget_para_list)):
        ...
        optimal_lambda = run_test(budget_para_list[i])
