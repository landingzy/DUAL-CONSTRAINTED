...
def bid(average_value, data, total_budget_list, budget_prop, base_bid, test_flag=False):
    # 数据统计
    real_imps = len(data)
    ...
    for day_index, day in enumerate(data.day.unique()):
        ...
        values = list(current_day_data['value'])
        market_prices = list(current_day_data['market_price'])
        time_frac = list(current_day_data['96_time_fraction'])
        ...
        day_win_value = 0  # 当天赢得价值
        day_real_pctr = np.sum(pctrs)
        day_real_value = np.sum(values)
        day_spend = 0  # 花费
        early_stop = False
        ...
        try:
            with tqdm(range(day_real_imps)) as tqdm_t:
                for impression_index in tqdm_t:
                    ...
                    bid_price = get_lin_bid(average_value, values[impression_index], base_bid)
                    if bid_price >= market_prices[impression_index] and day_spend + market_prices[
                        impression_index] <= today_budget and (
                            values[impression_index] - market_prices[impression_index]) / (
                            1e-4 + market_prices[impression_index]) >= roi:
                        day_win_pctr += pctrs[impression_index]
                        day_win_value += values[impression_index]

                        day_utility = day_utility + (values[impression_index] - market_prices[impression_index])

                        day_win_imps += 1
                        day_win_clks += clks[impression_index]
                        day_spend += market_prices[impression_index]
                        if test_flag:
                            bid_action.append([pctrs[impression_index], values[impression_index], bid_price,
                                               market_prices[impression_index],
                                               clks[impression_index], minutes[impression_index], 1,
                                               (values[impression_index] - market_prices[impression_index]),
                                               (values[impression_index] - (market_prices[impression_index])) / (
                                                           1e-4 + market_prices[impression_index])])
                        ...
                        if test_flag:
                            bid_action.append([pctrs[impression_index], values[impression_index], bid_price,
                                               ...
                                               (values[impression_index] - market_prices[impression_index]), (
                                                           values[impression_index] - (market_prices[
                                                       impression_index])) / (1e-6+market_prices[impression_index])])
                    if not early_stop:
        ...
        win_value += day_win_value
        win_utility += day_utility
        day_cpm = day_spend / (day_win_imps+1e-6)
        day_cpm_list.append(day_cpm)
        day_cpc = day_spend / max(1, day_win_clks)
        day_cpc_list.append(day_cpc)
        ...
        day_result_temp = [day, day_average_pctr, budget_prop, day_win_clks, day_real_clks,
                           ...
                           day_spend, today_budget, day_cpm, day_cpc, base_bid, day_win_pctr, day_win_value,
                           day_utility, day_real_pctr, day_end_time]

        day_result.append(day_result_temp)

    cpm = np.sum(spend) / win_imps
    cpc = np.sum(spend) / win_clks
    total_result = [average_value, budget_prop, win_clks, real_clks, win_imps, real_imps,
                    budget, spend, cpm, cpc, day_cpm_list, day_cpc_list, base_bid, win_pctr, win_value, win_utility,
                    end_time]
    if test_flag:
...
def train(cfg, test_flag=False):
    ...
    average_value = np.sum(train_data.value) / len(train_data)

    for budget_prop in cfg['budget_prop_list']:
        for base_bid in base_bid_list:
            print([camp, budget_prop, base_bid])
            train_result_temp, day_train_result_temp = bid(average_value, train_data, total_budget_list, budget_prop,
                                                           base_bid)
    ...
    total_train_log = pd.DataFrame(data=train_result,
                                   columns=['average_value', 'budget_prop', 'win_clks', 'real_clks', 'win_imps',
                                            'real_imps', 'budget',
                                            'spend', 'cpm', 'cpc', 'day_cpm_list', 'day_cpc_list', 'base_bid',
                                            'win_pctr', 'win_value', 'win_utility', 'end_time'])
    ...
    day_train_log = pd.DataFrame(data=day_train_result,
                                 columns=['day', 'day_average_pctr', 'budget_prop', 'day_win_clks', 'day_real_clks',
                                          'day_win_imps', 'day_real_imps',
                                          'day_spend', 'today_budget', 'day_cpm', 'day_cpc', 'base_bid', 'day_win_pctr',
                                          'day_win_value', 'day_utility',
...
def test(cfg):
    ...
    for budget_prop in cfg['budget_prop_list']:
        ...
        average_value = best_base_bid_row.iloc[0].average_value
        print([camp, budget_prop, best_base_bid])
        test_result_temp, day_test_result_temp, bid_action = \
            bid(average_value, test_data, total_budget_list, budget_prop, best_base_bid, test_flag=True)
        ...
        bid_action_df = pd.DataFrame(data=bid_action,
                                     columns=['pctr', 'value', 'bid', 'market_price', 'clk', 'minutes', 'purchase',
                                              'utility', 'real_ROI'])
    ...
    pd.DataFrame(data=test_result,
                 columns=['average_pctr', 'budget_prop', 'win_clks', 'real_clks', 'win_imps',
                          'real_imps',
                          'budget', 'spend', 'cpm', 'cpc', 'day_cpm_list', 'day_cpc_list', 'base_bid', 'win_pctr',
                          'win_value', 'win_utility', 'end_time']).to_csv(
...
if __name__ == '__main__':
    ...
    if config['train_type'] == 'reverse':
        config['train_data'] = 'test.csv'
        config['test_data'] = 'train.csv'
    else:
        config['train_data'] = 'train.csv'
        config['test_data'] = 'test.csv'
