...
class RLB_DP_I:
    ...
    def calc_Dnb(self, N, B, max_bid, m_pdf, save_path):
        print(get_time() + "\tD(n, b), N={}, B={}, save in {}".format(N, B, save_path))
        ...
        for n in range(1, N):
            ...
            V = nV[:]
            if flag:
                print(get_time() + "\tround {} end with early stop.".format(n))
            else:
                print(get_time() + "\tround {} end.".format(n))
        for b in range(0, B):
            dtb = V[b + 1] - V[b]
            if abs(dtb) < self.zero_precision:
                dtb = 0
    ...
    def run(self, auction_in, bid_log_path, N, c0, max_bid, input_type="file reader", delimiter=" ",
        ...
        for line in auction_in:
            if input_type == "file reader":
                ...
                (click, price, theta,value) = line
            a = self.bid(n, b, theta, max_bid)
            a = min(int(a), min(b, max_bid))

            log = get_time() + "\t{}\t{}_{}\t{}_{}_{}\t{}_{}\t".format(
                episode, b, n, a, price, click, clk, imp,value)
            if save_log:
                bid_list.append(a)
                log_in.write(log + "\n")

            if a >= price and value/(1e-3+price)>=1.5:
                imp += 1
                if click == 1:
                    clk += 1
                b -= price
